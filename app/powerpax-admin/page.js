    ...
    <div className="dashboard-container" style={{maxWidth: '1200px', margin: '0 auto', padding: '20px'}}>
      <header className="dashboard-header" style={{marginBottom: '30px'}}>
        ...
        <div className="header-right" style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => setIsAuthenticated(false)} style={{padding: '8px 15px', fontSize: '12px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold'}}>
      ...
      {selectedCompanyForms && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
          <div className="card" style={{ width: '100%', maxWidth: '900px', maxHeight: '85vh', overflowY: 'auto', position: 'relative' }}>
            ...
            <div className="modal-content" style={{ maxWidth: '1000px', width: '95%', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
              {/* Left Sidebar inside the Modal */}
              <div style={{ flex: '0 0 280px', background: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <h2 style={{marginTop: 0, marginBottom: '0', fontSize: '20px'}}>{selectedCompanyForms[0].auth_company_name || selectedCompanyForms[0].company_name}</h2>
                <div className="note" style={{ background: '#e0e7ff', color: '#4338ca', padding: '5px 10px', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold', display: 'inline-block', width: 'fit-content' }}>
                ...
                <button 
                  ...
                  style={{background: '#1e40af', padding: '12px', width: '100%', display: 'flex', gap: '8px', alignItems: 'center', justifyContent: 'center', fontSize: '13px', borderRadius: '6px'}}
                >
                  <i className="fas fa-file-pdf"></i> Download Letter
                </button>
                <button className="btn-gray" onClick={() => setSelectedCompanyForms(null)} style={{ padding: '10px', width: '100%', borderRadius: '6px' }}>Close View</button>
              </div>

              {/* Right Content Area (Scrollable Forms Data) */}
              <div style={{ flex: '1' }}>
                ...
                {Object.values(selectedCompanyForms.reduce((acc, sub) => {
                  ...
                  <div key={sub.id} style={{marginBottom: '20px', border: '1px solid #e2e8f0', background: '#f8fafc', padding: '20px', borderRadius: '8px'}}>
