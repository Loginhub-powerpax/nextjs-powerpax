...
export const generateParticipationLetter = async (exhibitorData) => {
  try {
    ...
    const displayCompanyName = capitalizeFirst(company_name || auth_company_name || 'Exhibitor Company');
    const displayContactPerson = capitalizeFirst(contactPerson || '[Exhibitor Name]');
    const displayAddress = capitalizeFirst(address || '[Address Line 1]');
    const displayLocation = capitalizeFirst(country || '[Country]');
    ...
    currentY -= 15;
    page.drawText("For Renewable Mirror / Tresub Media Pvt. Ltd.", { x: 60, y: currentY, size: 11, font: fontBold, color: textColor });
